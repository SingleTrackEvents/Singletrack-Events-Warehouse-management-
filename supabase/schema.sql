...
create table if not exists public.records (
  ...
  event_id       text,
  destination_id text,
  -- Monotonic; the client's pull cursor. BY DEFAULT rather than ALWAYS because
  -- push_records assigns it explicitly when a row changes, so that an updated
  -- row moves to the end of the queue and devices already past its old position
  -- are told about it. ALWAYS forbids that assignment outright.
  seq            bigint generated by default as identity,
  primary key (table_name, id)
);

-- Existing deployments were created with GENERATED ALWAYS, which rejects the
-- assignment above with 'column "seq" can only be updated to DEFAULT' — so the
-- first push of a record worked and every later change to it failed.
do $$
begin
  if exists (
    ...
    alter table public.records alter column seq set generated by default;
  end if;
end $$;
